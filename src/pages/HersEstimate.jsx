...
function CustomerSection({ leads, selectedLead, selectedLeadId, jobAddress,
  ...
  async function saveNew() {
    ...
    // Compares the last 10 digits of each number so formatting differences
    // don't matter, but requires an actual match - not just "one number
    // happens to contain some other number's digits somewhere in it",
    // which was matching unrelated phone numbers by pure coincidence the
    // more customers existed (same bug fixed in ProjectEstimate.jsx).
        ...
        <div style={CARD}>
          ...
          {isEditing && Number(unitCount)>1 && (
            <div style={{fontSize:11,color:C.faint,marginTop:8}}>
              Open Measurements to enter each unit — a unit switcher is there.
            </div>
          )}
          {!isEditing && Number(unitCount)>1 && (
            <div style={{fontSize:11,color:C.faint,marginTop:8}}>
