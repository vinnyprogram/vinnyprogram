...
export default function Settings() {
  ...
  const [offersGc, setOffersGc] = useState(false);
  ...
  async function load() {
    ...
    const { data:co } = await supabase.from("companies")
      .select("offers_insulation,offers_hers,offers_board_plaster,offers_gc").eq("id",company.id).maybeSingle();
    if(co){
        ...
        {tab==="trades" && (
          <div>
            <div style={{background:C.white,borderRadius:10,border:`1px solid ${C.border}`,padding:"20px 20px",marginBottom:12}}>
              ...
              <label style={{display:"flex",alignItems:"flex-start",gap:14,padding:"14px 16px",
                  border:`1px solid ${C.border}`,borderRadius:8,marginTop:10,cursor:"pointer",
                  background:offersGc?"#f0fdf4":"#fafafa"}}>
                <input type="checkbox" checked={offersGc}
                  onChange={e=>setOffersGc(e.target.checked)}
                  style={{width:18,height:18,marginTop:1,accentColor:C.green,flexShrink:0}} />
                <div>
                  <div style={{fontWeight:700,fontSize:14,color:C.ink}}>🏗️ General Contractor</div>
                  <div style={{fontSize:12,color:C.muted,marginTop:2}}>
                    Multi-trade estimates - measured areas, scopes and subcontractors, and a
                    shared materials catalog you can build from a starter list or your own entries.
                  </div>
                </div>
              </label>

              {!offersInsulation && !offersHers && !offersBoardPlaster && !offersGc && (
